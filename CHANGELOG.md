...
### Fixed
- `panel_set_widget` no longer stops answering for the whole 30 s relay window when its stale-combo recovery meets a node-def refresh: the recovery's fallback to the full frontend refresh (taken when the /object_info already fetched for authorization cannot key the target's concrete type) passed no `joinMs`, so the one deadline the refresh coalescer takes was never armed and the write waited unbounded — on a run something else started, or on one of its own. `graph_set_widget` now takes a 25 s command budget on its first line, so the baseline-seed wait, the authorization /object_info, that refresh and the #387 upload-asset probe all draw from ONE window instead of composing past it. When the budget runs out waiting, the reply says so with a structured `combo_refresh_incomplete: "refresh_still_running"` and a refusal that no longer claims the option list was refreshed — the run is not cancelled, so the retry it asks for joins work already in progress (#1413)
