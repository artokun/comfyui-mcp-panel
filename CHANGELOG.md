...
### Fixed
- **The model picker surfaces the newest advertised Opus as a first-class entry instead of a raw "Custom model" row (#377).** When the connected orchestrator's SDK still resolves the `opus` alias to the previous `claude-opus-4-8` while advertising the newer `claude-opus-5` as an unnamed pinned id, the picker collapsed neither — it showed the stale 4.8 alias as "Opus" and rendered `claude-opus-5` as an unrecognized "Custom model". The catalog logic (now factored into `web/js/lib/model-catalog.js` and unit-tested) is family-aware: each Claude family collapses to its newest advertised version, a surviving pinned `claude-*` id gets a clean label derived from the id itself (`claude-opus-5` → "Opus 5"), and the default pre-selection prefers the newest Opus — all derived from the authoritative advertised ids, with no hardcoded model→label map. The existing behaviors are preserved: an up-to-date SDK whose `opus` alias already resolves to the newest model keeps the alias and drops the pinned duplicate, and Fable (advertised only as a pinned `claude-fable-5[1m]`) still survives (#70).
- **Claude readiness no longer false-positives on empty OAuth credentials (#378).** `_provider_auth("claude")` reported Claude authenticated whenever `~/.claude/.credentials.json` merely existed — even when `claudeAiOauth.accessToken` and `refreshToken` were both empty (as left after `claude` signs out), so the panel offered Claude sessions that then failed authentication. It now parses the file and requires a genuinely-present access or refresh token; a missing/malformed/non-object file reads as not-signed-in rather than falsely ready.
